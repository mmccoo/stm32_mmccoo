...
// https://cloud.mail.ru/public/c6d69303181c%2FIIC-OLED%20module%20FZ1109%20FZ1112%20FZ1113%20.rar
...
// another implementation that I haven't tried is here:
// https://github.com/ARMmbed/mbed-hal-st-stm32cubef4/blob/master/source/stm32f4xx_hal_i2c__FIX.c
...
void WriteOledCmd(unsigned char I2C_Command) {
  uint8_t addr = 0x78;
  uint8_t pData[] = { 0x00, I2C_Command };
  HAL_StatusTypeDef iicstatus __attribute__ ((unused));
  iicstatus = HAL_I2C_Master_Transmit(&hi2c1, addr, pData, sizeof(pData), 10);
}

void WriteOledData(unsigned char I2C_Data)//Ð´Êý¾Ý
{
  uint8_t addr = 0x78;
  uint8_t pData[] = { 0x40, I2C_Data };
  HAL_StatusTypeDef iicstatus __attribute__ ((unused));
  iicstatus = HAL_I2C_Master_Transmit(&hi2c1, addr, pData, sizeof(pData), 10);
}
...
void OLED_Fill(unsigned char fill_Data)//È«ÆÁÌî³ä
{
  unsigned char m,n;
  for(m=0;m<8;m++) {
    WriteOledCmd(0xb0+m);		//page0-page1
    WriteOledCmd(0x00);		//low column start address
    WriteOledCmd(0x10);		//high column start address
    for(n=0;n<128;n++) {
      WriteOledData(fill_Data);
    }
  }
}
...
void InitOled(void)
{
	...
	WriteOledCmd(0xff); //ÁÁ¶Èµ÷½Ú 0x00~0xff
	WriteOledCmd(0xa1); //--set segment re-map 0 to 127
	WriteOledCmd(0xa6); //--set normal display
	WriteOledCmd(0xa8); //--set multiplex ratio(1 to 64)
	WriteOledCmd(0x3F); //
	WriteOledCmd(0xa4); //0xa4,Output follows RAM content;0xa5,Output ignores RAM content
	...
	switch(TextSize)
	{
		...
		case 1:
		{
			while(ch[j] != '\0')
			{
				...
				{
					x = 0;
					y+=2; // this font is a double row.
				}
				OLED_SetPos(x,y);
				for(i=0;i<8;i++)
					WriteOledData(F8X16[c*16+i]);
				OLED_SetPos(x,y+1);
				for(i=0;i<8;i++)
					WriteOledData(F8X16[c*16+i+8]);
				x += 8;
				j++;
			}
		}break;
